...
contract FregCoin is ERC1155, ERC1155Burnable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // Token ID for the FregCoin (ERC1155 single token type)
    uint256 public constant FREG_COIN = 1;

    string public name = "FregCoin";
    string public symbol = "FREGCOIN";
    ...
    function spin() external nonReentrant {
        require(balanceOf(msg.sender, FREG_COIN) >= 1, "No FregCoin");
    ...
    function airdrop(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            _mint(recipients[i], FREG_COIN, amounts[i], "");
            emit CoinsMinted(recipients[i], amounts[i]);
        }
    }

    // Simple airdrop: 1 FregCoin to each address
    function airdropOne(address[] calldata recipients) external onlyOwner {
        for (uint256 i = 0; i < recipients.length; i++) {
            _mint(recipients[i], FREG_COIN, 1, "");
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        require(tokenId == FREG_COIN, "Invalid token ID");
    ...
    function _encodeMetadata() internal pure returns (string memory) {
        bytes memory json = abi.encodePacked(
            '{"name": "FregCoin",',
            '"description": "Spin the wheel for a chance to win prizes! Burn 1 FregCoin to spin.",',
    ...
    function _encodeCoinSVG() internal pure returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="100" y="90" text-anchor="middle" fill="#8B4513" font-size="24" font-weight="bold">FREG</text>',
            '<text x="100" y="120" text-anchor="middle" fill="#8B4513" font-size="18">COIN</text>',
            '</svg>'
        );
